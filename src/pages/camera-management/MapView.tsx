        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <Card className="h-[600px]">
              ...
              <CardContent>
                <div className="relative w-full h-[500px] bg-slate-100 rounded-lg border border-border overflow-hidden">
                  {/* Site layout background */}
                  <div className="absolute inset-0 bg-gradient-to-br from-green-50 to-blue-50">
                    {/* Building outline */}
                    <div className="absolute top-10 left-10 w-80 h-60 border-4 border-slate-400 bg-white/80 rounded-lg shadow-lg">
                      <div className="p-2 text-sm font-medium text-slate-700">Main Building</div>
                    </div>
                    
                    {/* Parking area */}
                    <div className="absolute top-10 right-10 w-40 h-60 border-4 border-slate-400 bg-slate-200/80 rounded-lg shadow-lg">
                      <div className="p-2 text-sm font-medium text-slate-700">Parking</div>
                    </div>
                    
                    {/* Entrance area */}
                    <div className="absolute bottom-10 left-10 w-80 h-20 border-4 border-slate-400 bg-yellow-100/80 rounded-lg shadow-lg">
                      <div className="p-2 text-sm font-medium text-slate-700">Entrance Gate</div>
                    </div>
                  </div>
              ...
              <CardContent>
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {filteredCameras.map((camera) => (
                    <div
                      key={camera.id}
                      className={`flex items-center justify-between p-2 rounded-lg border cursor-pointer hover:bg-accent transition-colors ${
                        selectedCamera?.id === camera.id ? 'bg-accent border-primary/50' : 'border-border'
                      }`}
                      onClick={() => setSelectedCamera(camera)}
                    >
